...
JDK1.8以后的HashMap：链表⻓度大于阈值时(默认为8)，若当前数组的⻓度小于64，那么会先进行数组扩容。然后将链表转化为红黑树，以减少搜索时间。
...
jdk1.7：判断size是否达到阈值(0.75\*数组长度)，且是否产生哈希冲突。若size达到阈值且产生哈希冲突，则先扩容为2倍，再添加元素。
...
如果数据迁移后，双向链表的结构小于等于6，会将红黑树重新转为双向链表的结构。
