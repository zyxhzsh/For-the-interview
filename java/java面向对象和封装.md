...
当本类中成员方法的局部变量和成员变量重名的时候，根据就近原则，优先使用局部变量。如果需要访问本类中的成员变量，需要使用this.成员变量。this关键字可以解决重名无法区分的问题。谁调用的方法，谁就是this。
...
static修饰的成员方法叫做静态方法。静态方法不属于对象，而是属于类。静态方法可以被继承，不能被重写。
