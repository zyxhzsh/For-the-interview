...
Mysql是一种关系型数据库管理系统。关系型数据库是指：将数据保存在不同的数据表中，表与表之间可以有关联关系。

sql结构化查询语言，定义了一种操作关系型数据库的规则。
...
（2）sql语句不区分大小写，关键字建议使用大写。但是表中的数据区分大小写，这和sql语句无关。当表中的字符串和sql语句大小写不同时，mysql仍能查出记录但oracle不会查出记录。mysql不严谨。
...
对视图进行增删改查，会影响到原表数据。(通过视图影响原表数据，不是直接操作的原表)可以对视图进行CRUD操作。
```
create view 视图名 as DQL语句; #创建视图
drop view 视图名; #删除视图
